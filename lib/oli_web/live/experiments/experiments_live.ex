  ...
  def render(assigns) do
    ...
    <div class="content">
      ...
      <ol class="list-decimal">
        <li>Create an Upgrade account at <a href={@user_url} class="link"><%= @user_url %></a></li>
        <li>
          Frome <a
            href={
              Routes.live_path(OliWeb.Endpoint, OliWeb.Resources.AlternativesEditor, @project_slug)
            }
            class="link"
          >Manage Alternatives</a>,
          create one "decision point" and add two or more conditions (i.e. options)
        </li>
        <li>
          Place instances of the decision point, with corresponding content for each condition, in pages in this course project
        </li>
        <li>
          <b>After</b>
          steps 1 - 3, generate and download Upgrade segment and experiment JSON files. Import those into Upgrade as
          a new segment and new experiment.
        </li>
      ...
      <%= if @has_decision_point do %>
        <a
          ...
          href={Routes.experiment_path(OliWeb.Endpoint, :segment_download, @project_slug)}
        >
          Download Segment JSON
        </a>
        <a
          class="btn btn-primary"
          href={Routes.experiment_path(OliWeb.Endpoint, :experiment_download, @project_slug)}
        >
          Download Experiment JSON
        </a>
      <% else %>
        <strong>
          You can only generate and download these Upgrade files after you have created a decision point
        </strong>
